...
void MemTable::Put(const Slice &key, const Slice &value,
    ...
    char *new_key =
        _arena.Allocate(key_length + size_length * 2 + value_length);
    ...
    std::memcpy(new_key + size_length * 2 + key_length, value.data(),
                value_length);
    _list.Put(new_key);
}
...
bool MemTable::Get(const Slice &key, const uint64_t sequence_num,
    ...
    if (_key_cmp(*res, lookup_key.c_str()) == 0) {
        uint64_t tag = *(reinterpret_cast<const uint64_t *>(*res + key_length +
                                                            size_length - 8));
        if (!(tag & kTypeValue)) {
            return false;
        }
        int value_length = *(
            reinterpret_cast<const size_t *>(*res + key_length + size_length));
